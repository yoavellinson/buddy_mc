...
class MonauralEulerHeunSamplerDAPS(EulerHeunSampler):
    """
    ...
    def get_DAPS_estimate(self, x_hat, t_hat, h_tilde, K=2):
        """
        Refine x0 estimate with data consistency.
        This replaces raw Tweedie estimate in the sampler.
        """

        # prior estimate from diffusion model
        with torch.enable_grad():
            x0_prior = self.get_Tweedie_estimate(x_hat, t_hat)
        ...
        if self.args.tester.posterior_sampling.constraint_speech_magnitude.use:
            ...
            x0_prior = x0_prior * (s_scale / (x0_prior.detach().std() + 1e-8))

        # DAPS variable: posterior x0 sample / estimate
        z = x0_prior.detach().clone().requires_grad_(True)

        # important: prior strength should depend on noise level
        sigma2 = t_hat**2 + 1e-8

        for _ in range(K):
            Z_spec = self.stft(z).squeeze()  # [F, T]

            Y_hat_spec = self.apply_ctf_stft(Z_spec, h_tilde.detach())
            y_hat = self.istft(Y_hat_spec, length=self.y.shape[-1])

            loss_data = 0.0
            loss_data = loss_data + self.lambda_stft * self.rec_loss_stft(
                Y_hat_spec,
                self.stft(self.y).squeeze()
            )
            loss_data = loss_data + self.lambda_sisdr * self.rec_loss_sisdr(
                y_hat,
                self.y
            )

            loss_prior = ((z - x0_prior.detach()) ** 2).mean()

            # loss = loss_data + self.zeta * loss_prior / sigma2
            loss = loss_data + self.zeta * loss_prior / (sigma2 + 1e-3)
            grad = torch.autograd.grad(loss, z)[0]

            grad_norm = torch.linalg.norm(grad) / (self.args.exp.audio_len ** 0.5)
            grad = grad / (grad_norm + 1e-8)

            step_size = self.args.tester.sampling_params.daps_lr
            z = z - step_size * grad
    ...
    def stepEM(self, x_i, t_i, t_iplus1, gamma_i,i, eps=1e-5):
        ...
        if self.args.tester.posterior_sampling.constraint_speech_magnitude.use:
            s_scale = self.args.tester.posterior_sampling.constraint_speech_magnitude.speech_scaling
            x0_prior = x0_prior * (s_scale / (x0_prior.detach().std() + 1e-8))
        X_den = self.stft(x0_prior).permute(2, 1, 0)
        ...
        if i < self.warmup_steps:
            x_den = x0_prior          # no DAPS
        elif t_hat > 0.05:
            x_den = self.get_DAPS_estimate(x_hat, t_hat, h_tilde, K=1)
        else:
            x_den = self.get_DAPS_estimate(x_hat, t_hat, h_tilde, K=2)
        ...
        if t_iplus1 !=0 and self.order == 2 and i>self.warmup_steps: #second order correction
            ...
            x_den = self.get_DAPS_estimate(x_prime, t_prime, h_tilde, K=1)
            ...
            score = self.Tweedie2score(x_den, x_prime.detach(), t_prime)
